    ...
    for iteration, batch in enumerate(gen):
        ...
        if local_rank == 0:
            pbar.set_postfix(**{'total_triple_loss': total_triple_loss / (iteration + 1),
                                ...
                                'lr': get_lr(optimizer)})    # 把各类参数显示在 进度条的后面
            pbar.update(1)    # 步进长度
...
### lfw验证
    if lfw_eval_flag:
        print("开始进行LFW数据集的验证。")
        labels, distances = [], []
        for _, (data_a, data_p, label) in enumerate(test_loader):    # image1, image2, is——same
            with torch.no_grad():
                ...
                out_a, out_p = model_train(data_a), model_train(data_p) # 此处模型没有设置mode参数，默认为predict模式
                dists = torch.sqrt(torch.sum((out_a - out_p) ** 2, 1))
            distances.append(dists.data.cpu().numpy())
            labels.append(label.data.cpu().numpy()) # 是不是一个人 issame 布尔值
